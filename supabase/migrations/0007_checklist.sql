...
-- ===================== SEED templates =====================
-- Manhã 10 itens (atualizado 2026-06-05 — template de abertura removido, substituído por itens de encerramento/handover).
-- Noite 10 itens. Dollar-quoting preserva aspas e quebras de linha. on conflict (turno,item_id) do nothing.

insert into public.checklist_templates (turno, ordem, item_id, horario_referencia, categoria, item, descricao) values
('Manhã', 0, 'manha_medicamentos',       '07:00', 'Medicamentos',       'RISCO DIRETO NO CLIENTE',   $desc$1. Atualizar encomendas (chegou / não chegou / cliente avisado).
...
('Manhã', 1, 'manha_entregas_trava',     '07:00', 'Entregas',           'TRAVA OPERACIONAL',         $desc$1. Finalizar entregas do dia ou justificar.
...
('Manhã', 2, 'manha_fechamento_moto',    '07:00', 'Entregas',           'Fechamento da Moto',        $desc$Preencher sistema da moto com entregador e guardar chaves em local pré estipulado.
Guardar moto no interior da loja.$desc$),
('Manhã', 3, 'manha_clientes_wa',        '07:00', 'Clientes e WhatsApp','EXPERIÊNCIA',               $desc$1. Responder conversas críticas (preço, disponibilidade, entrega).
...
('Manhã', 4, 'manha_loja_higiene',       '07:00', 'Loja e Higiene',     'PRONTO PRA ABRIR',          $desc$1. Limpar e organizar balcão.
...
('Manhã', 5, 'manha_equipamentos',       '07:00', 'Equipamentos',       'NÃO TRAVAR AMANHÃ',         $desc$1. Encerrar PDVs/computadores corretamente.
...
('Manhã', 6, 'manha_seguranca',          '07:00', 'Segurança',          'RISCO ZERO',                $desc$1. Trancar cofre e armários sensíveis (incluindo controlados).
...
('Manhã', 7, 'manha_caixa_financeiro',   '07:00', 'Caixa e Financeiro', 'TRAVA DE SAÍDA',            $desc$1. Conferir caixa físico vs. sistema.
...
('Manhã', 8, 'manha_handover_jogo',      '07:00', 'Handover',           'PASSAR O JOGO LIMPO',       $desc$1. Registrar todas as pendências reais.
...
('Manhã', 9, 'manha_validacao',          '07:00', 'Validação final',    'Validação final (30–60s)',   $desc$Responder sem hesitar:
...
-- ANTIGOS 23 itens de abertura removidos. Mantidos apenas para referência no CHECKLIST_TEMPLATES.md
('Manhã', 99, 'manha_climatizacao', '07:00', 'Estrutura e Ambiente', 'Climatização', $desc$• Ligar o ar-condicionado.
